...
func (s *RewardsService) DoApproveMintReward(ctx *gin.Context) error {
	...
	currentSeason, err := model.GetCurrentSeason(s.Db)
	if err != nil {
	...
	if err != nil {
		if errors.Is(err, bigcache.ErrEntryNotFound) {
			...
			mintResult, err := data_srv.CalcMintRewards(ctx, s.Db, currentSeason)
			if err != nil {
	...
	cityHallProject, err := model.GetCityHallProject(s.Db)
...
func (s *RewardsService) DoSnapshotSeed(ctx *gin.Context) (int64, error) {
	// Get current reward records
	//user, enforcer, db, _ := api.ForContext(ctx)
	user, _, _, _ := api.ForContext(ctx)
	...
	err = s.Db.Save(currentSeason).Error
