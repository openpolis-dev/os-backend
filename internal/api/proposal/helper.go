...
func SaveProposalToMetaforo(db *gorm.DB, origProposalRecordId uint, voteType int, metaforoAccessToken string, EditorType int, metaforoGroupName string) error {
	...
	if origProposalRecord.ProposalRecordId != "" {
		...
		voteStartTime := time.Unix(origProposalRecord.CreateTs, 0).UTC().Add(origProposalRecord.PublicityDuration())
		voteEndTime := time.Unix(origProposalRecord.CreateTs, 0).UTC().Add(origProposalRecord.PublicityDuration() + origProposalRecord.VoteDuration())
